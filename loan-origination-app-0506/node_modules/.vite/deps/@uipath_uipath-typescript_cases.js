import { n as LoggerProvider, t as BatchLogRecordProcessor } from "./BatchLogRecordProcessor-BdjX946V.js";
//#region node_modules/@uipath/uipath-typescript/dist/cases/index.mjs
/******************************************************************************
...
function isOrchestratorError(error) {
	return typeof error === "object" && error !== null && "message" in error && "errorCode" in error && typeof error.message === "string" && typeof error.errorCode === "number";
}
function isEntityError(error) {
	return typeof error === "object" && error !== null && "error" in error && typeof error.error === "string";
}
function isPimsError(error) {
	return typeof error === "object" && error !== null && "type" in error && "title" in error && "status" in error && typeof error.type === "string" && typeof error.title === "string" && typeof error.status === "number";
}
/**
...
*/
var HttpHeaders = { X_REQUEST_ID: "x-request-id" };
/**
...
*/
var ErrorNames = { ABORT_ERROR: "AbortError" };
/**
* Parser for Orchestrator/Task error format
*/
var OrchestratorErrorParser = class {
	canParse(errorBody) {
...
var PimsErrorParser = class {
	...
	parse(errorBody, response) {
		...
		if (error.errors && Object.keys(error.errors).length > 0) {
			const errorMessages = Object.entries(error.errors).map(([field, messages]) => `${field}: ${messages.join(", ")}`).join("; ");
...
var GenericErrorParser = class {
	...
	parse(errorBody, response) {
		return {
			...
			code: response?.status?.toString(),
			details: { originalResponse: errorBody }
		};
	}
};
/**
...
*/
var ErrorResponseParser = class {
	constructor() {
		this.strategies = [
			new OrchestratorErrorParser(),
			new EntityErrorParser(),
			new PimsErrorParser(),
			new GenericErrorParser()
		];
	}
	/**
	...
	*/
	async parse(response) {
		try {
			...
			return this.strategies.find((s) => s.canParse(errorBody)).parse(errorBody, response);
		} catch {
			...
			return {
				...
				requestId: response.headers.get(HttpHeaders.X_REQUEST_ID) || void 0
			};
		}
	}
};
var errorResponseParser = new ErrorResponseParser();
/**
* Base error class for all UiPath SDK errors
* Extends Error for standard error handling compatibility
*/
var UiPathError = class extends Error {
...
*/
var AuthenticationError = class extends UiPathError {
	constructor(params = {}) {
...
* - Invalid scope
*/
var AuthorizationError = class extends UiPathError {
	constructor(params = {}) {
...
* - Invalid data format
*/
var ValidationError = class extends UiPathError {
	constructor(params = {}) {
...
* - Resource deleted
*/
var NotFoundError = class extends UiPathError {
	constructor(params = {}) {
...
* - API throttling
*/
var RateLimitError = class extends UiPathError {
	constructor(params = {}) {
...
* - Gateway timeout
*/
var ServerError = class extends UiPathError {
	constructor(params = {}) {
	...
	*/
	get isRetryable() {
		return this.statusCode === HttpStatus.BAD_GATEWAY || this.statusCode === HttpStatus.SERVICE_UNAVAILABLE || this.statusCode === HttpStatus.GATEWAY_TIMEOUT;
	}
};
/**
...
* - Request aborted
*/
var NetworkError = class extends UiPathError {
	constructor(params = {}) {
...
var ErrorFactory = class {
	...
	*/
	static createFromHttpStatus(statusCode, errorInfo) {
		...
		switch (statusCode) {
			case HttpStatus.BAD_REQUEST: return new ValidationError({
				message,
				statusCode,
				requestId
			});
			case HttpStatus.UNAUTHORIZED: return new AuthenticationError({
				message,
				statusCode,
				requestId
			});
			case HttpStatus.FORBIDDEN: return new AuthorizationError({
				message,
				statusCode,
				requestId
			});
			case HttpStatus.NOT_FOUND: return new NotFoundError({
				message,
				statusCode,
				requestId
			});
			case HttpStatus.TOO_MANY_REQUESTS: return new RateLimitError({
				message,
				statusCode,
				requestId
			});
			default:
				if (statusCode >= HttpStatus.INTERNAL_SERVER_ERROR) return new ServerError({
					message,
					statusCode,
					requestId
				});
	...
	*/
	static createNetworkError(error) {
		...
		if (error instanceof Error) if (error.name === ErrorNames.ABORT_ERROR) message = ErrorMessages.REQUEST_ABORTED;
		else if (error.message.includes("timeout")) message = ErrorMessages.REQUEST_TIMEOUT;
		else message = error.message;
		return new NetworkError({ message });
	}
};
var FOLDER_KEY = "X-UIPATH-FolderKey";
var FOLDER_ID = "X-UIPATH-OrganizationUnitId";
var TRACEPARENT = "traceparent";
var UIPATH_TRACEPARENT_ID = "x-uipath-traceparent-id";
/**
...
var ApiClient = class {
	...
	*/
	...
	async request(method, path, options = {}) {
		...
		const traceparentValue = `00-${crypto.randomUUID().replace(/-/g, "")}-${crypto.randomUUID().replace(/-/g, "").slice(0, 16)}-01`;
		const headers = {
			...defaultHeaders,
		...
		let body = void 0;
		if (options.body) body = isFormData ? options.body : JSON.stringify(options.body);
		try {
			...
			if (options.responseType === RESPONSE_TYPES.BLOB) return await response.blob();
			if ((headers["Accept"] || headers["accept"]) === CONTENT_TYPES.XML) return await response.text();
...
function processODataArrayResponse(oDataResponse, successData) {
	if (oDataResponse.value.length === 0) return {
		success: true,
		data: successData
	};
	return {
...
function decodeBase64(base64) {
	...
	} else bytes = new Uint8Array(Buffer.from(base64, "base64"));
...
*/
var PaginationManager = class PaginationManager {
	/**
	...
	static createCursor({ pageInfo, type }) {
		...
		switch (type) {
			case PaginationType.OFFSET:
				if (pageInfo.currentPage) cursorData.pageNumber = pageInfo.currentPage + 1;
				break;
			case PaginationType.TOKEN:
				if (pageInfo.continuationToken) cursorData.continuationToken = pageInfo.continuationToken;
				else return;
				break;
		}
		return { value: encodeBase64(JSON.stringify(cursorData)) };
	}
	/**
	...
	*/
	static createPaginatedResponse({ pageInfo, type }, items) {
		...
		if (pageInfo.currentPage && pageInfo.currentPage > 1) {
			...
			previousCursor = { value: encodeBase64(JSON.stringify(prevCursorData)) };
		}
		let totalPages = void 0;
		if (pageInfo.totalCount !== void 0 && pageInfo.pageSize) totalPages = Math.ceil(pageInfo.totalCount / pageInfo.pageSize);
...
function createHeaders(headersObj) {
	const headers = {};
	for (const [key, value] of Object.entries(headersObj)) if (value !== void 0 && value !== null) headers[key] = value.toString();
	return headers;
}
/**
...
*/
var PROCESS_INSTANCE_TOKEN_PARAMS = {
...
function transformData(data, fieldMapping) {
	if (Array.isArray(data)) return data.map((item) => transformData(item, fieldMapping));
	const result = { ...data };
	for (const [sourceField, targetField] of Object.entries(fieldMapping)) if (sourceField in result) {
...
function transformCaseKeys(data, convertCase) {
	if (Array.isArray(data)) return data.map((item) => {
		if (item === null || typeof item !== "object" || typeof item === "string") return item;
		return transformCaseKeys(item, convertCase);
	});
	const result = {};
	for (const [key, value] of Object.entries(data)) {
		const transformedKey = convertCase(key);
		if (value !== null && typeof value === "object") result[transformedKey] = transformCaseKeys(value, convertCase);
		else result[transformedKey] = value;
	}
	return result;
}
/**
...
function _mapFieldValue(obj, field, valueMap) {
	...
	return {
		...obj,
		[field]: lookupKey in valueMap ? valueMap[lookupKey] : obj[field]
	};
}
/**
...
function applyDataTransforms(data, options) {
	let result = data;
	if (options?.field && options?.valueMap) result = _mapFieldValue(result, options.field, options.valueMap);
...
function addPrefixToKeys(obj, prefix, keys) {
	const result = {};
	for (const [key, value] of Object.entries(obj)) if (keys.includes(key)) result[`${prefix}${key}`] = value;
...
var DEFAULT_ITEMS_FIELD = "value";
/** Default field name for total count in a paginated response */
var DEFAULT_TOTAL_COUNT_FIELD = "@odata.count";
/**
...
function getLimitedPageSize(pageSize) {
	if (pageSize === void 0 || pageSize === null) return DEFAULT_PAGE_SIZE;
	return Math.max(1, Math.min(pageSize, MAX_PAGE_SIZE));
}
/**
* Helper functions for pagination that can be used across services
*/
var PaginationHelpers = class PaginationHelpers {
	/**
	...
	*/
	static hasPaginationParameters(options = {}) {
		...
		return cursor !== void 0 || pageSize !== void 0 || jumpToPage !== void 0;
	}
	/**
	...
	*/
	static parseCursor(cursorString) {
		try {
			return JSON.parse(decodeBase64(cursorString));
		} catch {
	...
	*/
	static validateCursor(paginationOptions, paginationType) {
		if (paginationOptions.cursor !== void 0) {
			if (!paginationOptions.cursor || typeof paginationOptions.cursor.value !== "string" || !paginationOptions.cursor.value) throw new Error("cursor must contain a valid cursor string");
			try {
				const cursorData = PaginationHelpers.parseCursor(paginationOptions.cursor.value);
				if (paginationType) {
					if (!cursorData.type) throw new Error("Invalid cursor: missing pagination type");
					if (cursorData.type !== paginationType) throw new Error(`Pagination type mismatch: cursor is for ${cursorData.type} but service uses ${paginationType}`);
				}
			} catch (error) {
				if (error instanceof Error) {
					if (error.message.startsWith("Invalid cursor") || error.message.startsWith("Pagination type mismatch")) throw error;
				}
	...
	*/
	static validatePaginationOptions(options, paginationType) {
		if (options.pageSize !== void 0 && options.pageSize <= 0) throw new Error("pageSize must be a positive number");
		if (options.jumpToPage !== void 0 && options.jumpToPage <= 0) throw new Error("jumpToPage must be a positive number");
		PaginationHelpers.validateCursor(options, paginationType);
		if (options.jumpToPage !== void 0 && paginationType === PaginationType.TOKEN) throw new Error("jumpToPage is not supported for token-based pagination. Use cursor-based navigation instead.");
	...
	*/
	static getRequestParameters(options, paginationType) {
		if (options.jumpToPage !== void 0) return filterUndefined({
			pageSize: options.pageSize,
			pageNumber: options.jumpToPage
		});
		if (!options.cursor) return filterUndefined({
			pageSize: options.pageSize,
			pageNumber: paginationType === PaginationType.OFFSET ? 1 : void 0
		});
		try {
	...
	*/
	static async getAllNonPaginated(params) {
		...
		const headers = folderId ? createHeaders({ [FOLDER_ID]: folderId }) : {};
		let response;
		if (method === HTTP_METHODS.POST) response = await serviceAccess.post(endpoint, additionalParams, { headers });
		else response = await serviceAccess.get(endpoint, {
			params: additionalParams,
			headers
		});
		const rawItems = Array.isArray(response.data) ? response.data : response.data?.[itemsField];
		const totalCount = Array.isArray(response.data) ? void 0 : response.data?.[totalCountField];
		const parsedItems = typeof rawItems === "string" ? JSON.parse(rawItems) : rawItems || [];
		return {
			items: transformFn ? parsedItems.map(transformFn) : parsedItems,
			totalCount
		};
	}
	/**
	...
	*/
	static async getAll(config, options) {
		const { folderId, pageSize, cursor, jumpToPage, ...restOptions } = options || {};
		const isPaginationRequested = PaginationHelpers.hasPaginationParameters(options || {});
		let processedOptions = restOptions;
		if (config.processParametersFn) processedOptions = config.processParametersFn(restOptions, folderId);
		...
		if (isPaginationRequested) return PaginationHelpers.getAllPaginated({
...
var getGlobalStore = () => {
	...
	if (!globalObj[REGISTRY_KEY]) globalObj[REGISTRY_KEY] = /* @__PURE__ */ new WeakMap();
...
*/
var SDKInternalsRegistry = class {
	static get store() {
	...
	*/
	static get(instance) {
		...
		if (!internals) throw new Error("Invalid SDK instance. Make sure to pass a valid UiPath instance to the service constructor.");
		return internals;
	}
};
	...
	*/
	...
	async request(method, path, options = {}) {
		switch (method.toUpperCase()) {
			case "GET": return this.get(path, options);
			case "POST": return this.post(path, options.body, options);
			case "PUT": return this.put(path, options.body, options);
			case "PATCH": return this.patch(path, options.body, options);
			case "DELETE": return this.delete(path, options);
			default: throw new Error(`Unsupported HTTP method: ${method}`);
		}
	}
	async requestWithSpec(spec) {
		if (!spec.method || !spec.url) throw new Error("Request spec must include method and url");
		return this.request(spec.method, spec.url, spec);
	}
	async get(path, options = {}) {
		return { data: await __classPrivateFieldGet(this, _BaseService_apiClient, "f").get(path, options) };
	}
	async post(path, data, options = {}) {
		return { data: await __classPrivateFieldGet(this, _BaseService_apiClient, "f").post(path, data, options) };
	}
	async put(path, data, options = {}) {
		return { data: await __classPrivateFieldGet(this, _BaseService_apiClient, "f").put(path, data, options) };
	}
	async patch(path, data, options = {}) {
		return { data: await __classPrivateFieldGet(this, _BaseService_apiClient, "f").patch(path, data, options) };
	}
	async delete(path, options = {}) {
		return { data: await __classPrivateFieldGet(this, _BaseService_apiClient, "f").delete(path, options) };
	}
	/**
	...
	preparePaginationRequestParams(paginationType, params, paginationConfig) {
		...
		switch (paginationType) {
			case PaginationType.OFFSET:
				...
				if (params.pageNumber && params.pageNumber > 1) requestParams[offsetParam] = (params.pageNumber - 1) * limitedPageSize;
				requestParams[countParam] = true;
				break;
			case PaginationType.TOKEN:
				const tokenPageSizeParam = paginationParams?.pageSizeParam || BUCKET_TOKEN_PARAMS.PAGE_SIZE_PARAM;
				const tokenParam = paginationParams?.tokenParam || BUCKET_TOKEN_PARAMS.TOKEN_PARAM;
				if (params.pageSize) requestParams[tokenPageSizeParam] = getLimitedPageSize(params.pageSize);
				if (params.continuationToken) requestParams[tokenParam] = params.continuationToken;
				break;
		}
	...
	determineHasMorePages(paginationType, info) {
		switch (paginationType) {
			case PaginationType.OFFSET:
				...
				if (info.totalCount !== void 0) return info.currentPage * effectivePageSize < info.totalCount;
				return info.itemsCount === effectivePageSize;
			case PaginationType.TOKEN: return !!info.continuationToken;
			default: return false;
		}
	}
};
_BaseService_apiClient = /* @__PURE__ */ new WeakMap();
/**
...
*/
var ApplicationInsightsEventExporter = class {
	...
	async sendToApplicationInsights(payload) {
		try {
			...
			if (!response.ok) console.debug(`Failed to send event telemetry: ${response.status} ${response.statusText}`);
		} catch (error) {
...
var telemetryClient = class TelemetryClient {
	constructor() {
		this.isInitialized = false;
	}
	static getInstance() {
		if (!TelemetryClient.instance) TelemetryClient.instance = new TelemetryClient();
	...
	*/
	initialize(config) {
		...
		if (config) this.telemetryContext = config;
		try {
	...
	setupTelemetryProvider(connectionString) {
		const processor = new BatchLogRecordProcessor(new ApplicationInsightsEventExporter(connectionString));
		this.logProvider = new LoggerProvider({ processors: [processor] });
	...
	*/
	createCloudUrl() {
		...
		if (!baseUrl || !orgId || !tenantId) return UNKNOWN;
...
function createTrackedFunction(originalFunction, nameOrOptions, fallbackName, opts) {
	return function(...args) {
		...
		if (opts.condition !== void 0) if (typeof opts.condition === "function") shouldTrack = opts.condition.apply(this, args);
		else shouldTrack = opts.condition;
		if (shouldTrack) {
...
function createParams(paramsObj = {}) {
	const params = {};
	for (const [key, value] of Object.entries(paramsObj)) if (value !== void 0 && value !== null) params[key] = value;
	return params;
}
/**
* Service for interacting with UiPath Maestro Cases
*/
var CasesService = class extends BaseService {
	/**
	...
	*/
	async getAll() {
		const params = createParams({ processType: ProcessType.CaseManagement });
		return ((await this.get(MAESTRO_ENDPOINTS.PROCESSES.GET_ALL, { params })).data?.processes || []).map((caseItem) => ({
			...caseItem,
	...
	*/
	extractCaseName(packageId) {
		...
		if (caseManagementIndex !== -1) return packageId.substring(caseManagementIndex + 15).replace(/-/g, " ");
		return packageId;
	}
};
__decorate([track("Cases.GetAll")], CasesService.prototype, "getAll", null);
/**
...
*/
var CaseAppConfigMap = { sections: "overview" };
/**
...
function createTaskMethods(taskData, service) {
	return {
		async assign(options) {
			...
			const assignmentOptions = "userId" in options && options.userId !== void 0 ? {
				taskId: taskData.id,
...
*/
var TaskService = class extends BaseService {
	constructor() {
		super(...arguments);
		/**
		...
		*/
		this.processTaskParameters = (options, folderId) => {
			...
			if (folderId) if (processedOptions.filter) processedOptions.filter = `${processedOptions.filter} and organizationUnitId eq ${folderId}`;
			else processedOptions.filter = `organizationUnitId eq ${folderId}`;
	...
	*/
	async create(task, folderId) {
		...
		return createTaskWithMethods(applyDataTransforms(transformData((await this.post(TASK_ENDPOINTS.CREATE_GENERIC_TASK, externalTask, { headers })).data, TaskMap), {
			field: "status",
			valueMap: TaskStatusMap
		}), this);
	}
	/**
	...
	*/
	async getUsers(folderId, options) {
		...
		const optionsWithFolder = {
			...options,
			folderId
		};
		return PaginationHelpers.getAll({
			serviceAccess: this.createPaginationServiceAccess(),
			getEndpoint: (folderId) => TASK_ENDPOINTS.GET_TASK_USERS(folderId),
			getByFolderEndpoint: TASK_ENDPOINTS.GET_TASK_USERS(folderId),
	...
	*/
	async getAll(options) {
		const endpoint = options?.asTaskAdmin ? TASK_ENDPOINTS.GET_TASKS_ACROSS_FOLDERS_ADMIN : TASK_ENDPOINTS.GET_TASKS_ACROSS_FOLDERS;
		const transformTaskResponse = (task) => {
			return createTaskWithMethods(applyDataTransforms(transformData(pascalToCamelCaseKeys(task), TaskMap), {
				field: "status",
				valueMap: TaskStatusMap
			}), this);
		};
	...
	*/
	async getById(id, options = {}, folderId) {
		...
		if (taskType && taskType in TASK_TYPE_ENDPOINTS) {
			if (!folderId) throw new ValidationError({ message: "folderId is required when taskType is provided" });
			return this.getByTaskType(id, folderId, taskType, restOptions);
		}
		const headers = createHeaders({ [FOLDER_ID]: folderId });
		const modifiedOptions = this.addDefaultExpand(restOptions);
		const apiOptions = addPrefixToKeys(modifiedOptions, ODATA_PREFIX, Object.keys(modifiedOptions));
		const transformedTask = transformData(pascalToCamelCaseKeys((await this.get(TASK_ENDPOINTS.GET_BY_ID(id), {
			params: apiOptions,
			headers
		})).data), TaskMap);
		const resolvedFolderId = folderId || transformedTask.folderId;
		if (transformedTask.type in TASK_TYPE_ENDPOINTS) return this.getByTaskType(id, resolvedFolderId, transformedTask.type, restOptions);
		return createTaskWithMethods(applyDataTransforms(transformedTask, {
			field: "status",
			valueMap: TaskStatusMap
		}), this);
	}
	...
	*/
	async assign(taskAssignments) {
		...
		const pascalOptions = camelToPascalCaseKeys({ taskAssignments: assignmentArray });
		return processODataArrayResponse(pascalToCamelCaseKeys((await this.post(TASK_ENDPOINTS.ASSIGN_TASKS, pascalOptions)).data), assignmentArray);
	}
	/**
	...
	*/
	async reassign(taskAssignments) {
		const assignmentArray = Array.isArray(taskAssignments) ? taskAssignments : [taskAssignments];
		const pascalOptions = camelToPascalCaseKeys({ taskAssignments: assignmentArray });
		return processODataArrayResponse(pascalToCamelCaseKeys((await this.post(TASK_ENDPOINTS.REASSIGN_TASKS, pascalOptions)).data), assignmentArray);
	}
	/**
	...
	*/
	async unassign(taskIds) {
		...
		const options = { taskIds: taskIdArray };
		return processODataArrayResponse(pascalToCamelCaseKeys((await this.post(TASK_ENDPOINTS.UNASSIGN_TASKS, options)).data), taskIdArray.map((id) => ({ taskId: id })));
	}
	/**
	...
	*/
	getByTaskType(id, folderId, taskType, options = {}) {
		...
		const extraParams = taskType === TaskType.Form ? {
	...
	*/
	async getTaskByTypeEndpoint(id, folderId, endpoint, extraParams = {}) {
		const headers = createHeaders({ [FOLDER_ID]: folderId });
		return createTaskWithMethods(applyDataTransforms(transformData((await this.get(endpoint, {
			params: {
				taskId: id,
				...extraParams
			},
	...
	*/
	addDefaultExpand(options) {
		...
		processedOptions.expand = processedOptions.expand ? `${DEFAULT_TASK_EXPAND},${processedOptions.expand}` : DEFAULT_TASK_EXPAND;
		return processedOptions;
	}
};
__decorate([track("Tasks.Create")], TaskService.prototype, "create", null);
__decorate([track("Tasks.GetUsers")], TaskService.prototype, "getUsers", null);
__decorate([track("Tasks.GetAll")], TaskService.prototype, "getAll", null);
__decorate([track("Tasks.GetById")], TaskService.prototype, "getById", null);
__decorate([track("Tasks.Assign")], TaskService.prototype, "assign", null);
__decorate([track("Tasks.Reassign")], TaskService.prototype, "reassign", null);
__decorate([track("Tasks.Unassign")], TaskService.prototype, "unassign", null);
__decorate([track("Tasks.Complete")], TaskService.prototype, "complete", null);
var CaseInstancesService = class extends BaseService {
	/**
	...
	*/
	async getAll(options) {
		...
		const transformCaseInstance = (item) => {
			return createCaseInstanceWithMethods(transformData(item, CaseInstanceMap), this);
		};
	...
	*/
	async getById(instanceId, folderKey) {
		const instanceWithMethods = createCaseInstanceWithMethods(transformData((await this.get(MAESTRO_ENDPOINTS.INSTANCES.GET_BY_ID(instanceId), { headers: createHeaders({ [FOLDER_KEY]: folderKey }) })).data, CaseInstanceMap), this);
	...
	*/
	async enhanceInstanceWithCaseJson(instance) {
		if (!instance.folderKey) return instance;
		try {
			...
			if (caseJson && caseJson.root) {
				const transformedCaseAppConfig = caseJson.root.caseAppConfig ? (() => {
					...
					if (transformed.overview) transformed.overview = transformed.overview.map(({ id: _, ...rest }) => rest);
					return transformed;
				})() : void 0;
				return {
					...instance,
	...
	*/
	async getCaseJson(instanceId, folderKey) {
		try {
			return (await this.get(MAESTRO_ENDPOINTS.CASES.GET_CASE_JSON(instanceId), { headers: createHeaders({ [FOLDER_KEY]: folderKey }) })).data;
		} catch {
			return null;
		}
	}
	...
	*/
	async close(instanceId, folderKey, options) {
		return {
			success: true,
			data: (await this.post(MAESTRO_ENDPOINTS.INSTANCES.CANCEL(instanceId), options || {}, { headers: createHeaders({ [FOLDER_KEY]: folderKey }) })).data
		};
	}
	/**
	...
	*/
	async pause(instanceId, folderKey, options) {
		return {
			success: true,
			data: (await this.post(MAESTRO_ENDPOINTS.INSTANCES.PAUSE(instanceId), options || {}, { headers: createHeaders({ [FOLDER_KEY]: folderKey }) })).data
		};
	}
	/**
	...
	*/
	async reopen(instanceId, folderKey, options) {
		...
		return {
			success: true,
			data: (await this.post(MAESTRO_ENDPOINTS.CASES.REOPEN(instanceId), requestBody, { headers: createHeaders({ [FOLDER_KEY]: folderKey }) })).data
		};
	}
	/**
	...
	*/
	async resume(instanceId, folderKey, options) {
		return {
			success: true,
			data: (await this.post(MAESTRO_ENDPOINTS.INSTANCES.RESUME(instanceId), options || {}, { headers: createHeaders({ [FOLDER_KEY]: folderKey }) })).data
		};
	}
	/**
	...
	*/
	async getExecutionHistory(instanceId, folderKey) {
		const transformedResponse = transformData((await this.get(MAESTRO_ENDPOINTS.CASES.GET_ELEMENT_EXECUTIONS(instanceId), { headers: createHeaders({ [FOLDER_KEY]: folderKey }) })).data, TimeFieldTransformMap);
		if (transformedResponse.elementExecutions && Array.isArray(transformedResponse.elementExecutions)) transformedResponse.elementExecutions = transformedResponse.elementExecutions.map((execution) => {
			const transformedExecution = transformData(execution, TimeFieldTransformMap);
			if (transformedExecution.elementRuns && Array.isArray(transformedExecution.elementRuns)) transformedExecution.elementRuns = transformedExecution.elementRuns.map((run) => transformData(run, TimeFieldTransformMap));
	...
	*/
	async getStages(caseInstanceId, folderKey) {
		const [executionHistoryResponse, caseJsonResponse] = await Promise.allSettled([this.getExecutionHistory(caseInstanceId, folderKey), this.getCaseJson(caseInstanceId, folderKey)]);
		...
		if (!caseJson || !caseJson.nodes) return [];
		const executionMap = this.createExecutionMap(executionHistory);
		const bindingsMap = this.createBindingsMap(caseJson);
		return caseJson.nodes.filter((node) => node.type !== CASE_STAGE_CONSTANTS.TRIGGER_NODE_TYPE).map((node) => this.createStageFromNode(node, executionMap, bindingsMap));
	}
	/**
	...
	*/
	createExecutionMap(executionHistory) {
		...
		if (executionHistory?.elementExecutions) for (const execution of executionHistory.elementExecutions) executionMap.set(execution.elementId, execution);
		return executionMap;
	}
	/**
	...
	*/
	createBindingsMap(caseJsonResponse) {
		...
		if (caseJsonResponse?.root?.data?.uipath?.bindings) {
			for (const binding of caseJsonResponse.root.data.uipath.bindings) if (binding.id) bindingsMap.set(binding.id, binding);
		}
		return bindingsMap;
	}
	/**
	...
	*/
	resolveBinding(value, bindingsMap) {
		if (typeof value === "string" && value.startsWith("=bindings.")) {
			const bindingId = value.substring(10);
	...
	*/
	processTasks(node, executionMap, bindingsMap) {
		if (!node.data?.tasks || !Array.isArray(node.data.tasks)) return [];
		return node.data.tasks.map((taskGroup) => {
			if (Array.isArray(taskGroup)) return taskGroup.map((task) => {
				const taskId = task.id;
				const taskExecution = taskId ? executionMap.get(taskId) : void 0;
				let taskName = task.displayName;
				if (!taskName && task.data?.name) taskName = this.resolveBinding(task.data.name, bindingsMap);
				return {
	...
	*/
	createStageFromNode(node, executionMap, bindingsMap) {
		...
		return {
			...
			sla: node.data?.sla ? transformData(node.data.sla, StageSLAMap) : void 0,
...
__decorate([track("CaseInstances.GetAll")], CaseInstancesService.prototype, "getAll", null);
__decorate([track("CaseInstances.GetById")], CaseInstancesService.prototype, "getById", null);
__decorate([track("CaseInstances.Close")], CaseInstancesService.prototype, "close", null);
__decorate([track("CaseInstances.Pause")], CaseInstancesService.prototype, "pause", null);
__decorate([track("CaseInstances.Reopen")], CaseInstancesService.prototype, "reopen", null);
__decorate([track("CaseInstances.Resume")], CaseInstancesService.prototype, "resume", null);
__decorate([track("CaseInstances.GetExecutionHistory")], CaseInstancesService.prototype, "getExecutionHistory", null);
__decorate([track("CaseInstances.GetStages")], CaseInstancesService.prototype, "getStages", null);
__decorate([track("CaseInstances.GetActionTasks")], CaseInstancesService.prototype, "getActionTasks", null);
//#endregion
...
//# sourceMappingURL=@uipath_uipath-typescript_cases.js.map