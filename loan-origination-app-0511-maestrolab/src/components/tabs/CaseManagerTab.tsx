...
import { completeMaestroActionTask } from '../../services/loanService';
...
export function CaseManagerTab({ data, stage, caseInstanceId, folderKey }: CaseManagerTabProps) {
...
function Row({
  ...
  const onApprove = async () => {
    ...
    if (!caseInstanceId || !folderKey) {
      toast('Cannot approve: case instance is not linked');
      return;
    }
    setApproveState('spinning');
    const hideTimer = window.setTimeout(() => setApproveState('done'), 2000);
    try {
      await completeMaestroActionTask(sdk, caseInstanceId, folderKey, 'Approve', stage);
      toast('Approved.');
    } catch (err) {
      window.clearTimeout(hideTimer);
      setApproveState('idle');
      toast(err instanceof Error ? `Approve failed: ${err.message}` : 'Approve failed');
    }
  };
